...
function getSubTopicDetails($batchid, $subject_id, $tag, $type)
{
    ...
    $data =  json_decode(file_getsuper_contents("https://api.penpencil.xyz/v2/batches/$batchid/subject/$subject_id/contents?page=1&contentType=$type&tag=$tag", getBaseHeaders()), true);
    $i = count($data['data']);
    ...
    $p = 2;
    while ($i < $data['paginate']['totalCount']) {
        $data =  json_decode(file_getsuper_contents("https://api.penpencil.xyz/v2/batches/$batchid/subject/$subject_id/contents?page=$p&contentType=$type&tag=$tag", getBaseHeaders()), true);
        $i = $i + count($data['data']);
